...
function readManifest(root) {
  // Every read of the manifest shares the rule, not only the callers that happen to check
  // first. `hasManifest` and `verifyPayload` refused a symlink while the parser still read
  // through it, which is exactly the disagreement the rule exists to prevent.
  // (review round 14: codex-1 MINOR, hermes-1 NIT, kimi-1 NIT.)
  const entryProblem = manifestEntryProblem(root);
  if (entryProblem) {
    return { ok: false, error: entryProblem, manifest: null };
  }
  let raw;
  try {
