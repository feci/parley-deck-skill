...
function preflightUninstallUnit(unit, context) {
  ...
  // Before existence, before ownership, before `--force`: a corrupt recorded selection must not
  // reach the filesystem at all. This is the CRITICAL's fail-closed edge on the deletion side.
  if (unit.markerProblem) {
    return { ok: false, skill: unit.skill, dest, action: "blocked", message: unit.markerProblem };
  }
  ...
  // Removability is deliberately NOT predicted here. See `quarantineName`.
  return null;
}
