...
function walkRawTarget(from, rawTarget, record) {
  let current = path.isAbsolute(rawTarget)
    ? path.parse(path.resolve(rawTarget)).root || path.sep
    : path.dirname(from);
  for (const part of rawTarget.split(/[\\/]+/).filter(Boolean)) {
    if (part === ".") continue;
    if (part === "..") {
...
function resolutionTouchpoints(dest) {
  ...
  for (const part of parts) {
    ...
    for (let step = 0; step < 64; step += 1) {
      ...
      const landed = walkRawTarget(hop, target, record);
      let targetStat = null;
      try {
        targetStat = fs.lstatSync(landed);
      } catch (_error) {
        targetStat = null;
      }
