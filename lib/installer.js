...
function doctorCommand(context) {
  ...
  return {
    ...
    ok: results.every((result) =>
      result.skills.every((skill) => skill.status === "valid" && (!skill.runtime || skill.runtime.ok))
    ),
    command: "doctor",
    targets: results
  };
...
function preflightSkillUnit(target, unit, context) {
  ...
  if (fs.existsSync(dest) && !fs.existsSync(markerPath(dest)) && !context.options.force) {
    return {
      ok: false,
      skill: unit.skill,
      dest,
...
function skillUnitStatus(unit) {
  ...
  return {
    ...
    runtime: ok ? runtimeAvailability(unit.dest) : null
  };
}
...
function runtimeAvailability(root) {
...
// Probed once per process: `doctor --target all` inspects dozens of units and the answer
...
function probePython3() {
  ...
  if (run.error || run.status !== 0) {
    pythonProbe = { found: false, major: 0, minor: 0 };
    return pythonProbe;
  }
  const [major, minor] = String(run.stdout).trim().split(".").map(Number);
  pythonProbe = { found: Number.isInteger(major), major: major || 0, minor: minor || 0 };
  return pythonProbe;
}
...
function writeResult(result, context) {
  ...
  if (result.command === "version") {
    ...
    if (!result.ok && result.command === "doctor") {
      ...
      const broken = skills.some((skill) => skill.status !== "valid");
      const unavailable = skills.some((skill) => skill.runtime && !skill.runtime.ok);
      const reasons = [];
      if (broken) reasons.push("missing or malformed");
      if (unavailable) reasons.push("installed but operationally unavailable");
