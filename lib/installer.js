...
function walkRawTarget(from, rawTarget, record, depth = 0) {
  ...
  if (path.isAbsolute(rawTarget)) {
    const root = path.parse(rawTarget).root || path.sep;
    current = root;
    parts = rawTarget.slice(root.length).split(/[\\/]+/).filter(Boolean);
  } else {
    current = path.dirname(from);
    parts = rawTarget.split(/[\\/]+/).filter(Boolean);
  }
...
function resolutionTouchpoints(dest) {
  ...
  for (const part of parts) {
    ...
    if (target !== null) walkRawTarget(logical, target, record);
  }
  return chains;
}
