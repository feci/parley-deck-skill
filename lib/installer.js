...
function markerAddonNames(coreDest) {
  ...
  for (const entry of marker.addons) {
    ...
    if (bad) {
      return { names: [], problem: `install marker records an unusable add-on selection — ${bad}` };
    }
    seen.add(entry);
    names.push(entry);
  }
...
function removeFleetAtomically(plan, context) {
  ...
  // A blocker anywhere in the plan means nothing is quarantined at all: the fleet gate.
