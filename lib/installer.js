...
const TARGETS = [
  ...
  {
    // ZCode (Z.AI). `.zcode/skills` is the user skill root: the literal appears in the
    // zcode-app-cli runtime bundle, and ~/.zcode/cli/config.json exposes skills.roots for
    // additional ones. Verified 2026-08-19 rather than inferred from the sibling targets.
    name: "zcode",
    kind: "zcode",
    skillDir: path.join(".zcode", "skills"),
    commands: ["zcode"]
  }
];
...
function syncProjectCommand(context) {
  ...
  try {
    existingMetadata = JSON.parse(fs.readFileSync(project.metadataPath, "utf8"));
  } catch {
    existingMetadata = null;
  }
  const metadata = buildProjectMetadata(context, project, existingMetadata);
  const actions = [
    {
      action: "write",
...
function projectStatus(context) {
  ...
  return {
    ...
    protocolMatchesPackaged: protocolSha256 && packagedProtocolSha256
      ? protocolSha256 === packagedProtocolSha256
      : null,
    // kimi-1/F4: nothing in the skill package read `protocolRole`, so a SOURCE repo — the one
    // place where the packaged copy is by definition the OLDER one (COOPERATION.md:839-840) — was
    // told to consider adopting it. Reading the field costs one lookup and is the difference
    // between advice and misdirection.
    protocolRole: metadata.value && typeof metadata.value.protocolRole === "string"
      ? metadata.value.protocolRole.trim().toLowerCase()
      : null
  };
}
...
function recommendedActions(runtimeInstalls, project) {
  ...
  if (project.protocolMatchesPackaged === false) {
    // On a `source` deck the comparison is advisory in the other direction: the live protocol is
    // upstream of the packaged snapshot, so "adopt the packaged updates" is backwards by
    // definition (kimi-1/F4, MINOR — a message that misstates its own effect).
    actions.push(project.protocolRole === "source"
      ? "This deck is protocolRole: source — its COOPERATION.md is upstream of the packaged copy. The difference is expected; do NOT adopt the packaged protocol here. Re-package from this deck when the protocol changes."
      : "Review the local COOPERATION.md changes before adopting packaged protocol updates.");
  }

  return actions;
}

// Fields this installer does not own. `sync-project` rebuilds version.json from a fixed key list
// and writes it wholesale, so anything the CLI put there was silently deleted -- including
// `protocolRole`, the field COOPERATION.md 9.0 and `parley preflight` both gate on, while
// `status` recommends running exactly this command (audit finding kimi-1/F2).
//
// A tool that rewrites a shared file must preserve the parts it does not understand.
const PRESERVED_PROJECT_METADATA_KEYS = ["protocolRole", "created"];

function buildProjectMetadata(context, project, existing) {
  const manifest = project.packaged.compatibilityManifest || {};
  const metadataSchema = manifest.projectMetadataSchema || 1;

  const preserved = {};
  for (const key of PRESERVED_PROJECT_METADATA_KEYS) {
...
function preflightSkillUnit(target, unit, context) {
  ...
  //
  // `unit.sourceRoot`, not `unit.addon.root`: the core has a manifest now and is not an add-on,
  // so this gate skipped it entirely. Measured — appending bytes to `skills/parley-deck/SKILL.md`
  // without regenerating made `--check` exit 1 while `install` returned ok, wrote all six units,
  // and copied the drifted core into every destination. That is verification item 5 failing on
  // the one unit whose drift nobody would catch downstream. (codex-1 MAJOR, review round 1.)
  if (unit.sourceRoot && addonManifest.hasManifest(unit.sourceRoot)) {
    const verified = addonManifest.verifyPayload(unit.sourceRoot);
    if (!verified.ok) {
      return {
        ok: false,
        skill: unit.skill,
...
function copyPayloadAtomically(dest, target, unit, context) {
  ...
  try {
    ...
    validateInstalledPayload(temp, unit.kind, undefined, {
      sourceHasManifest: Boolean(unit.sourceRoot && addonManifest.hasManifest(unit.sourceRoot)),
      // The staged tree IS our shape, so the copy plan governs it.
...
function skillUnitStatus(unit, options) {
  ...
  const validation = validateInstalledPayload(unit.dest, unit.kind, { collect: true }, {
    ...
    packageRoot: state.present ? unit.packageRoot : null,
    sourceRoot: unit.sourceRoot
  });
  ...
  // Ownership is evidence from the MARKER, and it is answered by the SAME predicate the
  // mutation paths use. Health lives in `status`, `missing` and `problems`; it does not get a
  // second, quieter vote here.
  //
  // Cycle 2 fixed only the source-damaged case and left the general form
  // (`payloadOk ? !unmanaged : false`) in place — while the comment above it claimed the
  // general principle. codex-1 found the gap the comment had already promised away: delete one
  // installed file, keep a valid marker, and `doctor` reports `managed: false` while an
  // unforced `uninstall` removes the very same tree. Reproduced. External automation was being
  // told an ordinary damaged install is unowned by the tool that will replace it without
  // `--force`. (codex-1 MINOR, review round 3.)
  //
  // `unmanaged` is retained as the narrower fact it always was: no marker at all, payload
  // proven from the packaged source. Such a tree is genuinely not ours.
  const ownedByMarker = installerOwnsDestination(unit.dest, unit.skill);
...
// Returns { files, problems }. Fails CLOSED: a copy-plan source that cannot be enumerated is a
// problem, never a silently shorter requirement list.
//
// The first version returned bare files and swallowed every read error. That inverted the fix:
// measured, deleting `plugin.json` from BOTH the installed core and the packaged source made
// the damaged install report `valid`, `managed: true`, `missing: []`, `problems: []` and
// `doctor` exit 0. The required list shrank to match whatever was still readable, so the more
// broken the package, the healthier every install looked. Found independently by codex-1,
// hermes-1 and kimi-1 in review round 1.
//
// `lstatSync`, not `statSync`: `copyRecursive` refuses symlinks in a payload, so a symlinked
// copy-plan source is a defect here too, and following it would also let a link cycle recurse
// forever. (hermes-1 NIT, same round.)
// The packaged skill directory's own files, POSIX-relative — what a foreign installer's verbatim
// copy contains. Fails closed the same way: an unreadable source is a problem, not a free pass.
function safeSourceFiles(sourceRoot) {
  try {
    const files = addonManifest.listPayloadFiles(sourceRoot);
    // A floor that can be zero is not a floor: an empty packaged skill directory would demand
    // nothing of the installed tree. Every skill has `SKILL.md` by definition — that is what
    // makes a directory a skill — so an empty listing is a defect in the package, not a licence.
    // (hermes-1 R2-3, review round 2.)
    if (files.length === 0) {
      return {
        files: ["SKILL.md"],
        problems: [`packaged source ${path.basename(sourceRoot)} is empty; the installed payload cannot be checked against it`]
      };
    }
    ...
    return {
      files: ["SKILL.md"],
      problems: [`packaged source ${path.basename(sourceRoot)} cannot be enumerated (${error.code || error.message})`]
    };
  }
}
...
function corePayloadFiles(packageRoot, kind) {
  ...
  const walk = (abs, rel) => {
    ...
    try {
      ...
      problems.push(`packaged source for ${rel} cannot be read (${error.code}); the installed payload cannot be checked against it`);
      return;
    }
    if (stat.isSymbolicLink()) {
      problems.push(`packaged source for ${rel} is a symbolic link, which the copy plan refuses`);
      return;
    }
    if (!stat.isDirectory()) {
      files.push(rel);
    ...
    let entries;
    try {
      entries = fs.readdirSync(abs).filter((entry) => entry !== ".DS_Store");
    } catch (error) {
      problems.push(`packaged source directory ${rel} cannot be listed (${error.code}); the installed payload cannot be checked against it`);
      return;
    }
    for (const entry of entries) {
...
// `proof` carries what the unit knows about its own packaged origin:
//   sourceHasManifest — does the packaged source ship a parley-addon.json
//   packageRoot       — set ONLY for a tree this installer owns; the copy plan governs it
//   sourceRoot        — the packaged skill directory, which is what a foreign copy contains
function validateInstalledPayload(root, kind, options, proof) {
  const { sourceHasManifest, packageRoot, sourceRoot } = proof || {};
  let required;
  const sourceProblems = [];
  if (kind === "addon") {
    required = ["SKILL.md"];
  } else if (packageRoot) {
    // Ours: hold it to everything the copy plan writes.
    const derived = corePayloadFiles(packageRoot, kind);
    required = derived.files;
    sourceProblems.push(...derived.problems);
  } else if (sourceRoot) {
    // Not ours. The copy plan describes OUR destination shape — `plugin.json` and
    // `gemini-extension.json` are staged in from the repository root, and antigravity gets a
    // second `skills/SKILL.md` — but a foreign installer copies `skills/parley-deck/`, and none
    // of those are inside it. The historical per-kind lists demanded them anyway, so on the
    // gemini and antigravity targets a byte-perfect foreign core stayed `malformed` while the
    // same copy passed on codex, purely because the codex list happened to name only files that
    // live in the source directory. (kimi-1 MINOR, review round 1.)
    //
    // The honest floor for a tree we did not stage is the packaged source's own file list. The
    // manifest proof above it does the real work; this only keeps an empty directory from
    // reaching it.
    const listed = safeSourceFiles(sourceRoot);
    required = listed.files;
    sourceProblems.push(...listed.problems);
  } else if (kind === "gemini") {
    // No package root and no source root: the no-context fallback. Keep the historical lists so
    // this path behaves exactly as it did rather than validating nothing.
  ...
  // Source-enumeration failures are health problems in their own right. Without this the
  // requirement list simply shrank and a damaged package made every install look healthier.
  const problems = [...sourceProblems, ...(kind === "addon" ? manifestProblems(root, sourceHasManifest) : [])];
  const ok = missing.length === 0 && problems.length === 0;
  if (options && options.collect) {
    // `sourceProblems` is reported separately as well as folded into `problems`, so callers can
    // distinguish "this tree is defective" from "this tree cannot be checked right now". Health
    // fails either way; ownership must not, because ownership is a fact about the marker.
    return { ok, missing, problems, sourceProblems };
  }
  if (missing.length > 0) {
...
module.exports = {
  InstallerError,
  // Exported so coverage tests can assert against the target REGISTRY rather than against
  // the install result they are checking - deriving both sides from the result made the
  // assertion circular (review round 1, codex-1 MAJOR).
  TARGETS,
  MARKER_FILE,
  PACKAGE_ROOT,
  parseArgs,
