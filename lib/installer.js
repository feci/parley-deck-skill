...
function installCommand(context) {
  ...
  // B5 says "preflight every unit AND DESTINATION before the first write". Preflighting inside
  // each target made that per-target: a blocker in the last of fourteen targets was found after
  // thirteen had already been installed. The whole plan is checked first, then written.
  // (review round 8, codex-1 MAJOR.)
  if (!context.options.dryRun) {
    const blocked = new Map();
    for (const target of targets) {
      for (const unit of targetSkillUnits(target, context)) {
        const blocker = preflightSkillUnit(target, unit, context);
        if (blocker) blocked.set(`${target.name}/${unit.skill}`, blocker);
      }
    }
    if (blocked.size > 0) {
      const actions = targets.map((target) => {
        const skills = targetSkillUnits(target, context).map(
          (unit) =>
            blocked.get(`${target.name}/${unit.skill}`) || {
              ok: false,
              skill: unit.skill,
              dest: unit.dest,
              action: "skipped",
              message: "Not attempted: another skill or target in this install failed preflight."
            }
        );
        const core = skills[0];
        return {
      ...
      return { ok: false, command: "install", dryRun: false, actions };
    }
  }
...
function uninstallCommand(context) {
  ...
  // Cycle 10 lifted install's preflight from per-target to fleet-wide and uninstall never got
  // the same treatment, so a blocker in the last of fourteen targets was found after thirteen
  // had already been deleted. A deletion is a write, and a partial fleet of deletions is the
  // same forbidden state as a partial fleet of installs. (review round 10, hermes-1 MAJOR.)
  if (!context.options.dryRun) {
    const blocked = new Map();
    for (const target of targets) {
      for (const unit of targetSkillUnits(target, context)) {
        const blocker = preflightUninstallUnit(unit, context);
        if (blocker) blocked.set(`${target.name}/${unit.skill}`, blocker);
      }
    }
    if (blocked.size > 0) {
      ...
      return { ok: false, command: "uninstall", dryRun: false, actions };
    }
  }
...
// What `uninstallSkillUnit` will refuse or fail on, asked before the first deletion. Absence is
// not a blocker: removing what is not there is the no-op the command already reports.
function preflightUninstallUnit(unit, context) {
  const dest = unit.dest;
  if (!pathEntryExists(dest)) return null;

  if (!context.options.force && !installerOwnsDestination(dest, unit.skill)) {
    return {
      ok: false,
      skill: unit.skill,
      dest,
  ...
  // `--force` overrides whose tree may be removed, never whether the removal can succeed.
  const undisposable = firstRemovalObstacle(dest);
  if (undisposable) {
    return { ok: false, skill: unit.skill, dest, action: "blocked", message: undisposable };
  }
  return null;
}
...
function targetSkillUnits(target, context) {
  ...
  const recorded = writing ? null : markerAddonNames(target.dest);
...
function preflightSkillUnit(target, unit, context) {
  ...
  // An existing destination is not merely replaced — the old tree is renamed aside and removed.
  // If that removal cannot succeed, the failure is fully predictable here, and discovering it
  // after the commit rename left the unit installed while the command called it `failed`.
  // (review round 10, kimi-1 MAJOR and codex-1 MAJOR.)
...
function installSkillUnit(target, unit, context) {
  ...
  try {
    ...
    const warning = copyPayloadAtomically(dest, target, unit, context);
    return {
      ok: true,
      skill: unit.skill,
      dest,
...
function uninstallTarget(target, context) {
  ...
  if (!context.options.dryRun && !context.options.force) {
    const blockers = units.filter(
      (unit) => pathEntryExists(unit.dest) && !installerOwnsDestination(unit.dest, unit.skill)
    );
    if (blockers.length > 0) {
...
function copyPayloadAtomically(dest, target, unit, context) {
  ...
  try {
    ...
    if (!pathEntryExists(dest) && pathEntryExists(backup)) {
  ...
  // Past this line the replacement is committed and correct on disk. Removing the old copy is
  // housekeeping: preflight established that it can be removed, but if it fails anyway the unit
  // is still installed, and throwing would report `failed` for a unit that succeeded — and turn
  // a complete fleet into a partial one on the strength of leftover debris.
  // (review round 10, codex-1 MAJOR.)
  if (pathEntryExists(backup)) {
    try {
      fs.rmSync(backup, { recursive: true, force: true });
    } catch (error) {
      return `installed, but the previous copy could not be removed (${error.code}): ${backup}`;
    }
  }
  return null;
}
...
// Mirror of `firstCopyObstacle`, pointed at the DESTINATION instead of the source: can the tree
// that is already there be disposed of? Both mutation paths depend on it — the install replace
// path renames the old tree aside and removes it, and uninstall removes it outright — and
// nothing asked. Four cycles preflighted whether a destination can be *created* (ancestor
// existence, type, permissions) and whether it may be *touched* (ownership); removability is a
// third question, and the one that predates all of them. (review round 10, kimi-1 MAJOR.)
//
// Node's recursive `rm` empties bottom-up, so it cannot unlink inside a 0555 directory — one
// chmodded subdirectory anywhere in the tree is enough. Checking the root's mode is therefore
// not sufficient; the whole tree is walked. Symlinks are not traversed, matching `rmSync`.
function firstRemovalObstacle(dest) {
  const parent = path.dirname(dest);
  try {
    fs.accessSync(parent, fs.constants.W_OK | fs.constants.X_OK);
  } catch (error) {
    return `Destination parent ${parent} is not writable (${error.code}); ${dest} cannot be replaced or removed.`;
  }
  ...
  while (stack.length > 0) {
    ...
    try {
      ...
      return `${current} cannot be inspected (${error.code}); ${dest} cannot be replaced or removed.`;
    }
    // Unlinking a file or a symlink needs permission on its parent, which the parent's own
    // visit already established. Only directories must themselves be enterable and writable.
    if (!stats.isDirectory()) continue;
    try {
      fs.accessSync(current, fs.constants.R_OK | fs.constants.W_OK | fs.constants.X_OK);
    } catch (error) {
      return `${current} cannot be emptied (${error.code}); ${dest} cannot be replaced or removed.`;
    }
