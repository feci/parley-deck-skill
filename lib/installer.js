...
function walkRawTarget(from, rawTarget, record, depth = 0) {
  ...
  if (path.isAbsolute(rawTarget)) {
    ...
    current = root;
    parts = rawTarget.slice(root.length).split(separators).filter(Boolean);
  } else {
    const lexicalParent = path.dirname(from);
    try {
      current = fs.realpathSync(lexicalParent);
    } catch (_error) {
      current = lexicalParent;
    }
