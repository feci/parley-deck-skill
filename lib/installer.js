...
// Filesystem identity, not string comparison. `realpath` on the parent had two measured false
// negatives: a symlinked runtime root whose `skills/` does not exist yet (both realpath calls
// fail, the fallback keys differ, and one target's core silently becomes the other's), and
// case-only spellings on a case-insensitive volume, where realpath preserves the caller's case.
// Walk to the nearest EXISTING ancestor, key it by dev/ino, and append the not-yet-created
// tail. (review round 14: codex-1 MAJOR, hermes-1 MINOR, kimi-1 MINOR.)
const CASE_INSENSITIVE_FS = process.platform === "darwin" || process.platform === "win32";

function physicalKey(dest) {
  const tail = [];
  ...
  for (;;) {
    ...
    if (stat) {
      const suffix = tail.map((s) => (CASE_INSENSITIVE_FS ? s.toLowerCase() : s)).join("/");
      return `${stat.dev}:${stat.ino}${suffix ? `/${suffix}` : ""}`;
    }
    const parent = path.dirname(dir);
    if (parent === dir) {
      // Nothing on this path exists, not even the root: fall back to the resolved string.
...
// The resolved physical path of a destination: the nearest existing ancestor's realpath with the
// not-yet-created tail appended. Used for CONTAINMENT, which a dev/ino key cannot express.
function resolvedDestination(dest) {
  const tail = [];
  let dir = path.resolve(dest);
  for (;;) {
    let real = null;
    ...
    if (real !== null) {
      const resolved = tail.length === 0 ? real : path.join(real, ...tail);
      return CASE_INSENSITIVE_FS ? resolved.toLowerCase() : resolved;
    }
    const parent = path.dirname(dir);
    if (parent === dir) {
...
// Every physical location CONSULTED while resolving a destination — the entry itself at each
// step, before following it. `resolvedDestination` records where a path ends up; this records
// what it goes through, which is what a later commit can destroy. Measured: with a plain
// symlink stored inside one managed destination and another runtime's home pointing at it, the
// final paths do not overlap, so the cycle-19 containment check was silent — and the later
// commit renamed the directory holding that symlink, so the earlier unit's destination ceased
// to exist while the transaction had already reported it `installed`.
// (review round 16, codex-1 MAJOR.)
// The identity of an ENTRY — where it sits — rather than of whatever it resolves to.
// `physicalKey` uses `stat`, which follows a link, so keying a symlink with it returns the
// target's identity and throws away the one fact that matters: which directory the link lives
// in. That regressed round 16's fix the moment touchpoints moved onto identity.
function physicalEntryKey(entry) {
  const resolved = path.resolve(entry);
  const base = path.basename(resolved);
  return `${physicalKey(path.dirname(resolved))}/${CASE_INSENSITIVE_FS ? base.toLowerCase() : base}`;
}

function resolutionTouchpoints(dest) {
  // Every physical location a resolution passes THROUGH. Two things were wrong before:
  //
  //   * only the symlink's own entry was recorded, never what it points at — so a chain whose
  //     entry sits outside both destinations but whose intermediate hop sits inside one was
  //     invisible. Measured: ok:true, the earlier unit's destination left dangling, its files
  //     orphaned at the chain's end. (review round 17, hermes-1 MAJOR.)
  //   * points were compared as realpath strings, which cannot see an APFS firmlink: `/private`
  //     and `/System/Volumes/Data/private` are the same objects and `realpath` preserves both
  //     spellings. (review round 17: codex-1 MAJOR, kimi-1.)
  //
  // Both are answered the same way: follow the chain, and express every hop as a physical key.
  const parts = path.resolve(dest).split(path.sep).filter(Boolean);
  const points = [];
  const seen = new Set();
  const record = (candidate) => {
    const key = physicalEntryKey(candidate);
    if (seen.has(key)) return;
    seen.add(key);
    points.push(key);
  };

  let logical = path.sep;
  for (const part of parts) {
    ...
    for (let step = 0; step < 64; step += 1) {
      let target;
      try {
        target = fs.readlinkSync(hop);
      } catch (_error) {
        break;
      }
      const absolute = path.isAbsolute(target) ? target : path.join(path.dirname(hop), target);
      record(absolute);
      let targetStat = null;
      try {
        targetStat = fs.lstatSync(absolute);
      } catch (_error) {
        targetStat = null;
      }
      if (!targetStat || !targetStat.isSymbolicLink()) break;
      hop = absolute;
    }
  }
  return points;
...
// Keys are `dev:ino` followed by "/"-joined tail segments, so containment is a prefix test on
// that shape rather than on a platform path.
function overlaps(a, b) {
  return a === b || a.startsWith(`${b}/`) || b.startsWith(`${a}/`);
}

// Returns a Map of unit -> blocker for every unit whose physical destination is shared.
function aliasedDestinations(plan) {
  const byKey = new Map();
  for (const { target, units } of plan) {
    for (const unit of units) {
      const key = physicalKey(unit.dest);
      if (!byKey.has(key)) byKey.set(key, []);
      byKey.get(key).push({ target, unit });
    }
  }
  const blocked = new Map();
  for (const [key, entries] of byKey) {
    if (entries.length < 2) continue;
    const where = entries.map(({ target, unit }) => `${target.name}/${unit.skill}`).join(", ");
    for (const { unit } of entries) {
      blocked.set(unit, {
        ...
        dest: unit.dest,
        action: "blocked",
        message: `Destination is shared by ${where} — they resolve to the same directory (${key}). Point them at separate paths and re-run.`
      });
    }
  }

  // Equality was never enough. Staging calls `mkdirSync(parent, {recursive:true})`, so one unit
  // can MATERIALIZE another unit's destination between planning and commit. Measured: a nested
  // pair returned ok:true with the inner unit reported `installed`, the outer `replaced`, the
  // inner install gone from disk, and the surviving marker naming the outer target — a
  // single-process false success with a partial fleet, the state B5 and the changelog say
  // cannot happen. No planned destination may equal, contain, or be contained by another.
  // (review round 15, codex-1 MAJOR.)
  const resolved = [];
  for (const { target, units } of plan) {
    for (const unit of units) {
      resolved.push({ target, unit, path: resolvedDestination(unit.dest) });
    }
  }
  for (const entry of resolved) {
    entry.key = physicalKey(entry.unit.dest);
    entry.touchpoints = resolutionTouchpoints(entry.unit.dest);
  }
  for (let i = 0; i < resolved.length; i += 1) {
    for (let j = i + 1; j < resolved.length; j += 1) {
      ...
      // Does either unit's resolution pass through the other's destination? Checked before the
      // final-path comparison because it is the case the final paths cannot express.
      const crosses =
        a.touchpoints.some((point) => point !== a.key && overlaps(point, b.key)) ||
        b.touchpoints.some((point) => point !== b.key && overlaps(point, a.key));
      if (crosses) {
        ...
        for (const entry of [a, b]) {
          ...
          blocked.set(entry.unit, {
            ...
            action: "blocked",
            message: `Destinations depend on each other: ${where} — resolving one passes through the other. Point them at independent paths and re-run.`
          });
        }
        continue;
      }

      // Containment on physical identity too: a firmlink respelling gives two different
      // realpath strings for one directory, but the same dev/ino anchor.
      if (a.key === b.key || !overlaps(a.key, b.key)) continue;
      const where = `${a.target.name}/${a.unit.skill} and ${b.target.name}/${b.unit.skill}`;
      for (const entry of [a, b]) {
        if (blocked.has(entry.unit)) continue;
        blocked.set(entry.unit, {
          ...
          dest: entry.unit.dest,
          action: "blocked",
          message: `Destination overlaps another in this plan: ${where} — one contains the other. Point them at separate paths and re-run.`
        });
      }
    }
  }
...
function copyPayloadAtomically(dest, target, unit, context) {
  ...
  try {
    ...
    validateInstalledPayload(temp, unit.kind, undefined, Boolean(unit.addon && addonManifest.hasManifest(unit.addon.root)));
...
function skillUnitStatus(unit, options) {
  ...
  const validation = validateInstalledPayload(
    ...
    unit.kind,
    { collect: true },
    Boolean(unit.addon && addonManifest.hasManifest(unit.addon.root))
  );
...
function validateInstalledPayload(root, kind, options, sourceHasManifest) {
  let required;
  if (kind === "addon") {
    required = ["SKILL.md"];
  } else if (kind === "gemini") {
...
// `skillUnitStatus`, and `uninstallSkillUnit`. Cycle 10 converted only the install path, which
