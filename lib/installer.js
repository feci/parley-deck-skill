...
function resolutionTouchpoints(dest) {
  // Only the SYMLINKS matter. A plain component is just the path walking down to its own
  // destination, and treating those as touchpoints flags every pair of siblings, since their
  // shared parent contains them both. A symlink is the only component that makes resolution
  // depend on something a different unit might own.
  ...
  for (const part of parts) {
    ...
    let parentReal;
    try {
      parentReal = fs.realpathSync(path.dirname(logical));
    } catch (_error) {
      parentReal = path.dirname(logical);
    }
    const point = path.join(parentReal, part);
    points.push(CASE_INSENSITIVE_FS ? point.toLowerCase() : point);
  }
  return points;
}
...
function aliasedDestinations(plan) {
  ...
  for (let i = 0; i < resolved.length; i += 1) {
    for (let j = i + 1; j < resolved.length; j += 1) {
      ...
      const crosses =
        a.touchpoints.some((point) => point !== a.path && overlaps(point, b.path)) ||
        b.touchpoints.some((point) => point !== b.path && overlaps(point, a.path));
      if (crosses) {
      ...
      if (a.path === b.path || !overlaps(a.path, b.path)) continue;
      const where = `${a.target.name}/${a.unit.skill} and ${b.target.name}/${b.unit.skill}`;
      for (const entry of [a, b]) {
        if (blocked.has(entry.unit)) continue;
        blocked.set(entry.unit, {
