...
function probePython3(env) {
  ...
  // Keyed on every variable that can change which interpreter answers, not on PATH alone:
  // PYTHONHOME and friends can move the answer without moving PATH. (round 3, kimi-1 NIT.)
  const key = ["PATH", "PYTHONHOME", "PYTHONPATH", "PYTHONEXECUTABLE", "VIRTUAL_ENV"]
    .map((name) => (effective[name] === undefined ? "\u0000" : String(effective[name])))
    .join("\u0001");
