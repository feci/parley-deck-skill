...
function skillUnitStatus(unit, options) {
  ...
  // Ownership is evidence from the MARKER, not from whether the packaged source can be read.
  // With the source damaged and the installed tree byte-perfect, `payloadOk` goes false on the
  // source problem alone and dragged `managed` down with it — so a tree carrying this
  // installer's own valid marker was reported unowned in the same JSON document that showed the
  // marker. kimi-1 measured the blast radius as reporting-only: an unforced
  // `uninstall --only parley-deck` still succeeds on that tree, because `installerOwnsDestination`
  // reads the marker from disk. Reporting still has to agree with itself.
  // (hermes-1 R2-2(b), co-signed by kimi-1, review round 2.)
  const sourceOnlyFailure = (validation.sourceProblems || []).length > 0;
  const ownershipOk = validation.missing.length === 0 &&
    problems.filter((problem) => !(validation.sourceProblems || []).includes(problem)).length === 0;
