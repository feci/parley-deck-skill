...
function probePython3(env) {
  ...
  const key = effective.PATH === undefined ? "\u0000inherit" : String(effective.PATH);
  ...
  if (run.error || run.status !== 0) {
    ...
    probe = { found: Number.isInteger(major), major: major || 0, minor: minor || 0 };
  }
  ...
  if (result.command === "version") {
    ...
    for (const install of result.runtimeInstalls) {
      ...
      for (const skill of (install.skills || []).slice(1)) {
        ...
        // `status` probes the runtime, so it must also report the answer. Printing only
        // `valid` made it disagree with `doctor` about the same directory.
        // (review round 2, codex-1 MINOR.)
        if (skill.runtime && !skill.runtime.ok) {
          context.stdout.write(`    unavailable: ${skill.runtime.detail}\n`);
        }
        for (const problem of skill.problems || []) {
