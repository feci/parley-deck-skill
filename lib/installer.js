...
function usage() {
  return [
    "Usage:",
    `  parley-deck-skill install [--target auto|all|${TARGETS.map((target) => target.name).join("|")}|generic] [--scope user|project] [--project <path>] [--dest <path>] [--force] [--dry-run] [--json]`,
    `  parley-deck-skill doctor [--target auto|all|${TARGETS.map((target) => target.name).join("|")}|generic] [--scope user|project] [--project <path>] [--dest <path>] [--json]`,
    `  parley-deck-skill uninstall [--target auto|all|${TARGETS.map((target) => target.name).join("|")}|generic] [--scope user|project] [--project <path>] [--dest <path>] [--force] [--dry-run] [--json]`,
    `  parley-deck-skill paths [--target auto|all|${TARGETS.map((target) => target.name).join("|")}|generic] [--scope user|project] [--project <path>] [--dest <path>] [--json]`,
...
function isRuntimeDetected(definition, context) {
  if (context.options.scope === "project") {
    ...
    return dirExists(path.join(projectRoot, definition.skillDir.split(path.sep)[0]));
  }
  ...
  if (dirExists(path.join(context.homeDir, definition.skillDir.split(path.sep)[0]))) {
    return true;
  }

  return definition.commands.some((command) => commandExists(command, context.env));
}
